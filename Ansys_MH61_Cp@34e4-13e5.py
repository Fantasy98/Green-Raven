...
#######################################################################
n = 0
for Re in Reynolds_Number_MH61:
    ...
    plt.xlim([0,1])
    plt.grid()
    tlt = "Ansys_MH61_Cp@Re{}".format(Re)
    for aoa in Angle_of_Attack[1:-2]:
        ...
        X = X/np.max(X)
        plt.plot(X,Cp,'o',markersize = 2,label = r"$\alpha$={}".format(aoa))
    plt.legend()
...
for aoa in Angle_of_Attack:
    ...
    plt.xlim([0,1])
    plt.grid()
    tlt = "Ansys_MH61_Cp@AOA={}".format(aoa)
    for Re in np.arange(len(Reynolds_Number_MH61)):
        
        Upper, Lower = lf.Load_Cp_Ansys(Model=turbulent_model,Airfoil=Airfoil,Reynolds_number=Reynolds_Number_MH61[Re],Alpha=aoa)
